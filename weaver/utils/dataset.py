...
class SimpleIterDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(self, file_dict, data_config_file,
        ...
        def _collate_bool_fn(batch, *, collate_fn_map=None):
            return torch.tensor([bool(x) for x in batch], dtype=torch.bool)
        default_collate_fn_map.update({ak.Array: _collate_awkward_array_fn, np.bool_: _collate_bool_fn, bool: _collate_bool_fn})
        if for_training:
